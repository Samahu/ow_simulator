...
static std::unordered_map<std::string, CompressionType> compression_types = { { "uncompressed", Uncompressed },
                                                                              { "bz2", BZ2 },
                                                                              { "lz4", LZ4 } };
...
template <>
CompressionType get_option<CompressionType>(const string& key, CompressionType defval)
{
  if (!ros::param::has(key))
    return defval;
  string value;
...
bool get_recorder_options(rosbag::RecorderOptions& out_options)
{
  try
  {
    ...
    // converted to bytes
    out_options.buffer_size = MEGABYTES_TO_BYTES * get_option("/bag_recorder_node/options/buffer_size",
                                                              out_options.buffer_size / MEGABYTES_TO_BYTES);
    out_options.chunk_size = KILOBYTES_TO_BYTES * get_option("/bag_recorder_node/options/chunk_size",
